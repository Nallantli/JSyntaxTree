...
public class DrawTree {
	...
	public DrawTree(Node NS, int fontSize, int spacingX, int spacingY, int border, String font_name, float strokeWeight,
			boolean curved_movement) {
		this.NS = NS;
	...
	public int paintNode(int _x, int _y, Graphics2D g2, Node n) {
		...
		if (!n.subNodes.isEmpty()) {
			...
			if (n.bracket == BRACKET.SQUARE_BRACKET) {
				...
				case 1:
					drawAlignedString(
							center_x - n.getWidth(this, g2) / 2
									+ (int) (JSyntaxTree.GetWidthOfAttributedString(getTrig("["), g2) * 0.5),
							bracket_y, g2, "[", Alignment.LEFT);
					drawAlignedString(
							center_x + n.getWidth(this, g2) / 2
									- (int) (JSyntaxTree.GetWidthOfAttributedString(getTrig("]"), g2) * 1.5),
							bracket_y, g2, "]", Alignment.LEFT);
					break;
				case 2:
					drawAlignedString(
				...
				default:
					...
					drawAlignedString(
							center_x - n.getWidth(this, g2) / 2
									+ (int) (JSyntaxTree.GetWidthOfAttributedString(getTrig("\u23a1"), g2) * 0.5),
							(int) (_y + fontSize * 1.5), g2, bl, Alignment.LEFT);
					drawAlignedString(
							center_x + n.getWidth(this, g2) / 2
									- (int) (JSyntaxTree.GetWidthOfAttributedString(getTrig("\u23a4"), g2) * 1.25),
							(int) (_y + fontSize * 1.5), g2, br, Alignment.LEFT);
				}
	...
	public void drawMovement(Node start, Node end, Graphics2D g2, boolean inOut) {
		...
		if (!start.subNodes.isEmpty()) {
			g2.draw(new Line2D.Float(startX - start.getWidth(this, g2) / 2, startY,
					startX - start.getWidth(this, g2) / 2, startY - fontSize / 2));
			g2.draw(new Line2D.Float(startX + start.getWidth(this, g2) / 2, startY,
					startX + start.getWidth(this, g2) / 2, startY - fontSize / 2));
			g2.draw(new Line2D.Float(startX - start.getWidth(this, g2) / 2, startY,
					startX + start.getWidth(this, g2) / 2, startY));
		}
		...
		if (curved_movement)
			g2.draw(new QuadCurve2D.Float(startX, startY, (startX + endX) / 2,
					depth_max + Math.max(spacingY, Math.abs(startY - endY)), endX, endY));
		else {
